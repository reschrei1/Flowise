...
class AzureOpenAIAssistant_Agents implements INode {
    label: string
    name: string
    version: number
    description: string
    ...
    constructor() {
        this.label = 'Azure OpenAI Assistant'
        this.name = 'azureopenAIAssistant'
        this.version = 4.0
    ...
    async clearChatMessages(nodeData: INodeData, options: ICommonObject, sessionIdObj: { type: string; id: string }): Promise<void> {
        ...
        if (!azureOpenAIApiKey) {
            options.logger.error(`AzureOpenAI ApiKey not found`)
            return
        }
...
const downloadImg = async (openai: AzureOpenAI, fileId: string, fileName: string, ...paths: string[]) => {
...
module.exports = { nodeClass: AzureOpenAIAssistant_Agents }
